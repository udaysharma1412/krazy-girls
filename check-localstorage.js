    ...
    <script>
        ...
        async function fetchMongoUsers() {
            ...
            try {
                ...
                if (result.success) {
                    let html = `<h3>📊 ${result.count} Users Found in MongoDB:</h3>`;
                    result.users.forEach((user, index) => {
                        html += `<div class="user">
                            ...
                            Welcome Email Sent: ${user.welcomeEmailSent || 'Not set'}<br>
                            ID: ${user._id}
                        </div>`;
                    });
                    mongoDiv.innerHTML = html;
                } else {
                    mongoDiv.innerHTML = '<p>❌ Failed to fetch users from MongoDB</p>';
                }
            } catch (error) {
                mongoDiv.innerHTML = `<p>❌ Error: ${error.message}</p>`;
            }
        }
